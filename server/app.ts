...
const CzujnikTemperatury = new Gpio(588, 'in', 'both');
...
CzujnikKuchnia.watch((err, value) => {
  ...
  elem.findOne({ gpio: 592 }).then((element) => {
    if (element && element.automation) {
      ...
      if (value === 1) {
        ...
        if (cameraProcess === null) {
          ...
          cameraProcess.on('close', (code) => {
            elem.updateOne(element, { $set: { value: true } });
            console.log(`Python process exited with code ${code}`);
            cameraProcess = null;
          });
        }
      } else if (value === 0) {
        // Ustawienie timera, kt�ry wy��czy kamer�, je�li przez 10 sekund nie b�dzie ruchu
        if (!cameraTimeout) {
          cameraTimeout = setTimeout(() => {
            if (cameraProcess != null) {
              cameraProcess.kill(); // Zako�cz proces kamery
              cameraProcess = null;
              elem.updateOne(element, { $set: { value: false } });
              console.log('Kamera zosta�a wy��czona z powodu braku aktywno�ci.');
            }
            cameraTimeout = null; // Resetowanie zmiennej timera
          }, 10000); // 10 sekund
        }
      }
    }
...
async function logUserActivity(userId, deviceId, state) {
  ...
  if (count === 0) {
    try {
      ...
      const interactableElements = await elem.find({ gpio: { $in: [592, 598, 593, 24, 583] } }).toArray();
...
const updateDeviceStatesForUsersWithAIEnabled = async () => {
  ...
  if (userSetting) {
    ...
    const devices = await elem.find({ gpio: { $in: [592, 598, 593, 24, 583] } }).toArray();
